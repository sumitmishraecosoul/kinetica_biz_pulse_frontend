...
interface PrivateLabelRowData {
  name: string;
  isTotal: boolean;
  isSubTotal?: boolean;
  children?: PrivateLabelRowData[];
  cases?: MetricData;
  gSales?: MetricData;
  fGP?: MetricData;
...
interface PrivateLabelTableProps {
  data: PrivateLabelRowData[];
  isLoading: boolean;
}
...
const renderRow = (row: PrivateLabelRowData, level: number = 0) => {
...
const PrivateLabelTable: React.FC<PrivateLabelTableProps> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-8 text-gray-600">
        No Private Label Brands data available for the selected filters.
      </div>
    );
  }
