...
interface FoodserviceSKUsRowData {
  name: string;
  isTotal: boolean;
  isSubTotal?: boolean;
  gSales?: MetricData;
...
interface FoodserviceSKUsTableProps {
  data: FoodserviceSKUsRowData[];
  isLoading: boolean;
}
...
const renderRow = (row: FoodserviceSKUsRowData) => {
...
const FoodserviceSKUsTable: React.FC<FoodserviceSKUsTableProps> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-8 text-gray-600">
        No Foodservice SKUs data available for the selected filters.
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
        <div className="space-y-1 text-xs text-gray-600">
          <p>- Food includes Food, Mell, and CAL business units.</p>
...
export default FoodserviceSKUsTable;
